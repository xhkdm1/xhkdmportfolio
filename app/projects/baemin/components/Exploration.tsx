    ...
    <section className="bg-white px-5 py-20 sm:px-8 sm:py-24 lg:px-12 lg:py-28">
      <div className="mx-auto w-full max-w-5xl">
        <header className="max-w-4xl">
          <p className="text-sm font-medium uppercase tracking-[0.16em] text-zinc-500">04 · Exploration</p>
        ...
        <p className="mt-14 text-xs font-medium uppercase tracking-[0.14em] text-[#008A95]">
          Step 1 · 해결 위치 탐색
        </p>
          ...
          {directions.map((direction) => (
            <article key={direction.title} className="rounded-3xl border border-zinc-200 bg-zinc-50 p-7 sm:p-8 lg:grid lg:h-full lg:grid-rows-[auto_auto_4.5rem_auto_1fr]">
              ...
              <div className="mt-6 border-t border-zinc-200 pt-4">
                <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#008A95]">고려한 이유</p>
              </div>
              ...
              <div className="mt-5 border-t border-zinc-200 pt-4 lg:mt-0">
                <p className="text-xs font-medium uppercase tracking-[0.12em] text-zinc-400">우려</p>
              </div>
          ...
          {concepts.map((concept, index) => (
            <article key={concept.label} className="overflow-hidden rounded-[2rem] border border-zinc-200 bg-white">
              <div className={`p-7 text-white sm:p-9 ${index === 0 ? "bg-zinc-950" : "bg-[#2AC1BC]"}`}>
                <p className="text-sm font-medium uppercase tracking-[0.16em] text-white/70">{concept.label}</p>
                <p className="mt-5 text-xl font-medium leading-8">{concept.description}</p>
              </div>
              <div className="p-7 sm:p-9">
                <p className="text-xs font-medium uppercase tracking-[0.14em] text-zinc-500">Trade-off</p>
